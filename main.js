...
ipcMain.on('PostMan', (e, val) => {
    ...
    var executablePath = dir + '\\PostMan.exe';
...
ipcMain.on('Crusher', (e, val) => {
    ...
    var executablePath = dir + '\\Closer.exe';
    var child = require('child_process').exec;
...
ipcMain.on('BigBang', (e, val) => {
    ...
    var executablePath = dir + '\\BigBang.exe';
    var child = require('child_process').exec;
...
ipcMain.on('Python', (e, val) => {
    ...
    var executablePath = dir + '\\build.exe';
    var executablePath1 = dir + '\\Network.exe';
    ...
    // exec('taskkill /f /t /im build.exe', (err, stdout, stderr) => {
    //     if (err) {
    //         console.log(err)
    //     }
    ...
    var converter = csv()
        .fromFile(data.filepath).then((JSON) => {
            ...
            if (data.radio == "PreProd") {
                var dir = fs.readdirSync('\\\\VA1PZCNAS01.us.ad.lfg.com\\sfg_preprod_build\\YAML_Current\\nonprod\\')
                var files = dir.filter(file => file.match(/^.*\.(yaml)$/ig))
                console.log(files.length)
                var doc1 = [];
                for (let n = 0; n < files.length; n++) {
                    if (files[n] != "GP4BurgerBusters2LFG.yaml") {
                        var doc = yaml.safeLoad(fs.readFileSync('\\\\VA1PZCNAS01.us.ad.lfg.com\\sfg_preprod_build\\YAML_Current\\nonprod\\' + files[n]), 'utf8')
                        doc1.push(doc)
            ...
            if (data.radio == "Prod") {
                var dir = fs.readdirSync('\\\\VA1PZCNAS01.us.ad.lfg.com\\sfg_preprod_build\\YAML_Current\\prod\\')
                var files = dir.filter(file => file.match(/^.*\.(yaml)$/ig))
                console.log(files.length)
                var doc1 = [];
                for (let n = 0; n < files.length; n++) {

                    var doc = yaml.safeLoad(fs.readFileSync('\\\\VA1PZCNAS01.us.ad.lfg.com\\sfg_preprod_build\\YAML_Current\\prod\\' + files[n]), 'utf8')
                    doc1.push(doc)
                }
            ...
            function validateEmail(email) {
                var re = email.indexOf("@");
                var me = email.lastIndexOf("@")
                if (re != me) {
            ...
            function MBXcheck() {
                for (let i = 0; i < data.CSVData.length; i++) {
                    ...
                    var g = LFGRC.includes(data.CSVData[i].RoutingChannelTemplate)
                    ...
                    var string = data.CSVData[i].RequesterEmailAddress
                    var requester = string.split(" ")

                    var string1 = data.CSVData[i].DetailEmail
                    var detail = string1.split(" ")

                    var string2 = data.CSVData[i].ConfirmEmail
                    var confirm = string2.split(" ")

                    var string3 = data.CSVData[i].FailEmail
                    var fail = string3.split(" ")
                    ...
                    if (data.CSVData[i].ProducerConsumer != "Producer" && data.CSVData[i].RoutingChannelTemplate != "LFG_Generic_Static_Routing_Secondary_MBX_2" && data.CSVData[i].RoutingChannelTemplate != "LFG_Generic_Static_Routing_Secondary_MBX_1") {
                        var el = files.some(m => m.includes(data.CSVData[i].Producer))
                        console.log(el)
                        if (el == false) {
                            var coverted = files.map(v => v.toLowerCase())
                            var lower = data.CSVData[i].Producer.toLowerCase()
                            var el = coverted.some(a => a.includes(lower))
                            if (el) {
                    ...
                    if (data.CSVData[i].ProducerConsumer == "Producer" || data.CSVData[i].ProducerConsumer == "producer") {
                        ...
                        if (data.CSVData[i].SSHProfileID != "" || data.CSVData[i]["Delivery Path"] != "" || data.CSVData[i].FileRename != "" || data.CSVData[i].DetailEmail != "" || data.CSVData[i].ConfirmEmail != "" || data.CSVData[i].FailEmail != "") {
                        ...
                        if (j < data.CSVData.length) {
                            ...
                            if (data.CSVData[i].Mailbox.slice(-2) == "_1" && data.CSVData[j].Mailbox.slice(-2) == "_2") {
                                ...
                                if (data.CSVData[i].Mailbox.slice(0, -2) == data.CSVData[j].Mailbox.slice(0, -2)) {
                                    ...
                                    var MB1 = data.CSVData[i].Mailbox
                                    var MB2 = data.CSVData[j].Mailbox

                                    var RC1 = data.CSVData[i].RoutingChannelTemplate
                                    var RC2 = data.CSVData[j].RoutingChannelTemplate
                    ...
                    if (data.radio == "Prod") {
                        ...
                        for (let o = 0; o < doc1.length; o++) {
                            ...
                            if (Boolean(doc1[o].ibm_b2b_trading_partner[data.CSVData[i].Mailbox])) {
                                console.log(doc1[o].ibm_b2b_trading_partner[0])
                        ...
                        if (data.CSVData[i].Community == "Prod_EDM" || data.CSVData[i].Community == "Prod_HR" || data.CSVData[i].Community == "Prod_LibertyMutual" || data.CSVData[i].Community == "Prod_NonStandard" || data.CSVData[i].Community == "Prod_RPS" || data.CSVData[i].Community == "SFGAdmin_Test") {
                    ...
                    var str = data.CSVData[i].Password
                    ...
                    if (data.CSVData[i].SSHProfileID == "891380141b85c553fnode1") {
                        ...
                        try {
                            ...
                            var dir = data.CSVData[i]["Delivery Path"].replace(/\\/g, "/")

                            var n = dir.includes("_")
...
app.on('window-all-closed', () => {
    if (true) {
        try {
            var dir1 = os.homedir() + "\\Molecule\\Crusher"
        ...
        try {
            var dir1 = os.homedir() + "\\Molecule\\BigBang"
            fs.readdir(dir1, (err, files1) => {
                if (err) {
                    console.log(err)
        ...
        try {
            var dir2 = os.homedir() + "\\Molecule\\CRs"
        ...
        try {
            var dir3 = os.homedir() + "\\Molecule\\PostOffice"
        ...
        try {
            var dir9 = os.homedir() + "\\Molecule\\Crusher"
            fs.removeSync(dir9)
            var dir8 = os.homedir() + "\\Molecule\\PostOffice"
            fs.removeSync(dir8)
            var dir7 = os.homedir() + "\\Molecule\\CRs"
            fs.removeSync(dir7)
            var dir10 = os.homedir() + "\\Molecule\\BigBang"
            fs.removeSync(dir10)
            var dir6 = os.homedir() + "\\Molecule"
            fs.removeSync(dir6)
